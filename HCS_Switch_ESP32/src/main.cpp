...
String ID = "10004";
String token = "McORX2KVsyh2YP7Sk0uiUMvUOzUJGAeGG8Ti7Mr0GHMFd0AdSuCyBOp4SKg1q4qm";
...
String postDataToServer(String url, String data) {
...
void loop() {
    if (connectionMode) {
        ...
        if (Serial2.available() > 0) {
            ...
            if(input == "x"){
            String JSONData = ID + "," + token ;
            JSONData.trim();
            String URL = basURL + "getSwitchStatus";

            Serial.println("POST TO " + URL + " with " + JSONData);
            String res = postDataToServer(URL, JSONData);
            res.trim();
            Serial2.println(res);
